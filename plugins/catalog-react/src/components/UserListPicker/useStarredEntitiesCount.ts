...
import { GetPaginatedEntitiesInitialRequest } from '@backstage/catalog-client';
...
export function useStarredEntitiesCount() {
  ...
  const refRequest = useRef<GetPaginatedEntitiesInitialRequest>();
  useMemo(async () => {
    ...
    const request: GetPaginatedEntitiesInitialRequest = {
      filter: {
        ...filter,
